    ...
    <div className="items-center justify-between hidden p-2 lg:flex">
      <div className="flex gap-5">
        {BannerLeftData.map((data, index) => (
          <Link key={index} href={data.href} className="flex items-center justify-center gap-2 text-sm font-semibold hover:text-rose-500"><span>{data.icon}</span>{data.title}</Link>
        ))}
      </div>
      <div className="flex gap-5">
